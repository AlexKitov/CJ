...
import Utilities.CustomLogger.TimeUnits._
import Utilities.Utils.now

object CustomLogger {
	def log[A](blockName: String, unit: TimeUnits = milli)(block: => A): A = {
	...
	case class Duration(nsStart: Long, nsEnd: Long){
		...
		def getDurationMessage(msg: String, unit: TimeUnits = nano): String = {
			unit match {
				...
				case TimeUnits.hms   => s"$now: $msg ${getHours  % 60} hours ${getMins  % 60} minutes ${getSecs % 60}.${getMilli % 1000} seconds"
			}
		}
